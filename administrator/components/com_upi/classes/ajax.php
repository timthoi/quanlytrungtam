...
* @subpackage	Class
*/
class UpiClassAjax extends JObject
{
	/**
	...
	*/
	public static function responseHook($options = array(), $version)
	{
		...
		switch(strtoupper($options['renderExceptions']))
		{
			...
			case 'HTML':
				$return->transaction->htmlExceptions = UpiClassView::renderMessages('HTML');
				break;

			case 'TEXT':
				$return->transaction->rawExceptions = UpiClassView::renderMessages('TEXT');
				break;
			...
			default:
				...
				else
				{
					...
					$return->transaction->exceptions = UpiClassView::renderMessages(null);
