...
* @subpackage	
*/
class UpiClassControllerItem extends JControllerForm
{
	/**
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_upi.' . $this->view_list . '.default',
			'com_upi.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	protected function applyRedirection($result, $redirections, $vars = array())
	{
		...
		switch($redirection)
		{
			...
			case 'stay':
				$this->setRedirect(UpiHelper::urlRequest($vars));
				return;
				break;
		...
		//Filters followers
		$model = CkJModel::getInstance($this->view_list, 'UpiModel');
		if ($model)
		{
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('UPI_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	public function redirect()
	{
		if ($this->redirect)
		{
			...
			if ($jinput->get('return') == 'json')
			{
				...
				UpiClassAjax::responseJson(array(
	...
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		...
		try
		{
			$changedValue = UpiHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
