...
* @subpackage	Classes
*/
class UpiModelCourseperiod extends UpiClassModelItem
{
	/**
	* View list alias
	*
	* @var string
	*/
	protected $view_item = 'courseperiod';
	...
	* @var string
	*/
	protected $view_list = 'courseperiods';
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'courseperiod', 'STRING');
	}
	...
	*/
	public function getTable($type = 'courseperiod', $prefix = 'UpiTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_upi.edit.courseperiod.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('courseperiod.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
				...
				$data->ordering = null;
				$data->published = null;
				$data->course_id = $jinput->get('filter_course_id', $this->getState('filter.course_id'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the courseperiod
	*
	* @return	void
	*/
	protected function prepareQuery(&$query, $pk)
	{
		...
		//FROM : Main table
		$query->from('#__upi_courseperiods AS a');
		...
		{
			case 'courseperiod.courseperiod':
				...
				//SELECT
				$this->addSelect('_course_id_.title AS `_course_id_title`');

				//JOIN
				$this->addJoin('`#__upi_courses` AS _course_id_ ON _course_id_.id = a.course_id', 'LEFT');
	...
	*/
	public function save($data)
	{
		...
		$data['end_date'] = UpiHelperDates::getSqlDate($data['end_date'], array('Y-m-d'), true, 'USER_UTC');
