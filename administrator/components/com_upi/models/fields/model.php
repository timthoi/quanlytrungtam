...
class JFormFieldModel extends JFormFieldGroupedList
{
	...
	*/
	protected function getGroups()
	{
		...
		$listGroups = UpiHelperForm::getElementGroups($this->element);
		...
		$items = UpiHelperForm::getItems($this);
		...
		foreach($items as $item)
		{
			...
			{
				$val = UpiHelper::parseValue($item, $labelKey);
				if ($val !== null)
	...
	*/
	protected function getOptions()
	{
		...
		if (!empty($items))
		foreach ($items as $item)
			$options[$item->$listKey] = UpiHelper::parseValue($item, $labelKey);
	...
	*/
	public function prepareModel(&$model)
	{
		...
		$groups = UpiHelperForm::getElementGroups($this->element);
