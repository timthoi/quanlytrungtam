...
* @subpackage	Classes
*/
class UpiModelPeriod extends UpiClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getTable($type = 'period', $prefix = 'UpiTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_upi.edit.period.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('period.id') == 0)
			{
				...
				$data->title = null;
				$data->alias = null;
				$data->weekday = $jinput->get('filter_weekday', $this->getState('filter.weekday'), 'STRING');
				$data->note = null;
				...
				$data->published = null;
				$data->ordering = null;
				$data->courseperiod_id = $jinput->get('filter_courseperiod_id', $this->getState('filter.courseperiod_id'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the period
	*
	* @return	void
	*/
	...
	*/
	public function save($data)
	{
		...
		$data['start_time'] = UpiHelperDates::getSqlDate($data['start_time'], array('H:i'), true, 'USER_UTC');

		//Convert from a non-SQL formated date (end_time)
		$data['end_time'] = UpiHelperDates::getSqlDate($data['end_time'], array('H:i'), true, 'USER_UTC');
