...
class UpiModelStaves extends UpiClassModelList
{
	...
	*/
	public function __construct($config = array())
	{
		...
		$this->set('filter_vars', array(
			...
			'certification_description' => 'date:Y-m-d',
			'note' => 'date:Y-m-d'
				));
	...
	*/
	protected function getStoreId($id = '')
	{
		...
		$id	.= ':'.$this->getState('filter.birthday');
		$id	.= ':'.$this->getState('filter.creation_date');
		$id	.= ':'.$this->getState('filter.certification_description');
		$id	.= ':'.$this->getState('filter.note');
	...
	*/
	protected function prepareQuery(&$query)
	{
		...
		$this->addSelect(	'a.id,'
						.	'a.created_by,'
						.	'a.published');
		$this->addSelect('concat(a.last_name," ",a.first_name) AS `full_name`');
		...
		if($filter_birthday = $this->getState('filter.birthday'))
		{
		if ($filter_birthday !== null){
				$this->addWhere("a.birthday = " . $this->_db->Quote($filter_birthday));
		}
		}
		...
		if($filter_certification_description = $this->getState('filter.certification_description'))
		{
		if ($filter_certification_description !== null){
				$this->addWhere("a.certification_description = " . $this->_db->Quote($filter_certification_description));
		}
		}

		if($filter_note = $this->getState('filter.note'))
		{
		if ($filter_note !== null){
				$this->addWhere("a.note = " . $this->_db->Quote($filter_note));
		}
		}

		//WHERE - SEARCH : search_search : search on First Name + Last Name + Address 1 + Address 2 + Home Phone + Mobile Phone
		$search_search = $this->getState('search.search');
		$this->addSearch('search', 'a.first_name', 'like');
		$this->addSearch('search', 'a.last_name', 'like');
		$this->addSearch('search', 'a.address_1', 'like');
		$this->addSearch('search', 'a.my_text', 'like');
		$this->addSearch('search', 'a.home_phone', 'like');
		$this->addSearch('search', 'a.mobile_phone', 'like');
