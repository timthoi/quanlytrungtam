...
* @subpackage	Classes
*/
class UpiModelClass extends UpiClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getTable($type = 'class', $prefix = 'UpiTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_upi.edit.class.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('class.id') == 0)
			{
				...
				$data->title = null;
				$data->alias = null;
				$data->subject_id = $jinput->get('filter_subject_id', $this->getState('filter.subject_id'), 'INT');
				...
				$data->tutor_id = $jinput->get('filter_tutor_id', $this->getState('filter.tutor_id'), 'INT');
				$data->room_id = $jinput->get('filter_room_id', $this->getState('filter.room_id'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the class
	*
	* @return	void
	*/
