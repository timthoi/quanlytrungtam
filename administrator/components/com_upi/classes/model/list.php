...
* @subpackage	Classes
*/
class UpiClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'UpiModel');
	...
	public function getItems()
	{
		try
		{
			...
			if ($error = $db->getErrorMsg()) {
				if (!$this->canAdmin())
					$error = JText::_('UPI_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'UpiModel');
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_upi', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = UpiHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'UpiModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__upi_' . $this->getName())
