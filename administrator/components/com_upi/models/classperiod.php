...
* @subpackage	Classes
*/
class UpiModelClassPeriod extends UpiClassModelItem
{
	/**
	* View list alias
	*
	* @var string
	*/
	protected $view_item = 'classperiod';
	...
	* @var string
	*/
	protected $view_list = 'classperiods';
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'classperiod', 'STRING');
	}
	...
	*/
	public function getTable($type = 'classperiod', $prefix = 'UpiTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_upi.edit.classperiod.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('classperiod.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
	...
	*/
	protected function prepareQuery(&$query, $pk)
	{
		...
		//FROM : Main table
		$query->from('#__upi_classperiods AS a');
		...
		{
			case 'classperiod.classperiod':

				//BASE FIELDS
				$this->addSelect(	'a.note,'
								.	'a.class_id');
				break;
			case 'all':
	...
	*/
	public function save($data)
	{
		...
		$data['register_date'] = UpiHelperDates::getSqlDate($data['register_date'], array('Y-m-d'), true, 'USER_UTC');
