...
class UpiHelperForm
{
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = UpiHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'upi.') . $relation->model;

				$model = UpiHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = UpiClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return UpiHelper::componentModel($model, $item);
	}
