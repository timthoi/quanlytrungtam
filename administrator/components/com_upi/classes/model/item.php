...
* @subpackage	Classes
*/
class UpiClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	* @var string
	*/
	protected static $extension = 'upi';
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("UPI_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_upi' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!UpiHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			...
			case 'courseperiod_id': return $tbl.$com. 'courseperiods';
			case 'course_id': return $tbl.$com. 'courses';
			case 'class_id': return $tbl.$com. 'classes';
			case 'staff_id': return $tbl.$com. 'staves';
			case 'period_id': return $tbl.$com. 'periods';
			case 'subject_id': return $tbl.$com. 'subjects';
			case 'grade_id': return $tbl.$com. 'grades';
			case 'teacher_id': return $tbl.$com. 'classteachers';
			case 'tutor_id': return $tbl.$com. 'classtutors';
			case 'room_id': return $tbl.$com. 'rooms';
			case 'type_test_id': return $tbl.$com. 'typetests';
			case 'studen_id': return $tbl.$com. 'students';
			case 'test_id': return $tbl.$com. 'tests';
			case 'student_id': return $tbl.$com. 'students';
			case 'test_class_id': return $tbl.$com. 'testclasses';	
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'UpiModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_UPI . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'UpiModel');
		if ($modelList)
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_UPI . '/models/forms');
		JForm::addFieldPath(JPATH_UPI . '/models/fields');
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'upiModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Upi"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_UPI . '/fork/models/forms';
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("UPI_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !UpiHelperDates::isNull((string)$value))
						{
							$time = UpiHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('UPI_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
